...
**Pago ou a comprar nos custos derivados.** Um preço que mora em outro registro não tem "pago" próprio, então há um terceiro recurso, `Payment` (`/payments`): uma marca `{link {type,id}, paid}`, uma por registro (índice único entre as vivas e `409 payment_exists`). Sem marca, vale o palpite (a data passou, ou o registro está `COMPLETED`/`VISITED`); com marca, ela manda nos dois sentidos, para quem pagou adiantado e para quem ainda não pagou o que já aconteceu. O cliente só grava uma marca quando ela contradiz o palpite, e a apaga quando volta a concordar, então a lista de marcas fica mínima. Marcar uma categoria ou a viagem inteira como paga é o cliente aplicando isso linha a linha.
